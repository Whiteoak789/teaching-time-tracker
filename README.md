...
Pushing a version tag such as `v1.0.3`, or manually running the **Release installers** workflow, builds both installers on GitHub-hosted macOS and Windows machines. The workflow keeps the release as a draft until both platform builds succeed, then publishes it with stable asset names.
